		...
		$.extend(this, {
			...
			gotoIndex: function(index, dontPause, bypassHistory) {
				...
				// #AH
                                ...
                                var regExp = /<div class="image-desc">(.*?)<\/div>/;
                                var matches = regExp.exec(imageData.caption.html());
				if (matches && matches[1]) {
				    matches[1] += ". <em>Thomas Gray Archive</em>. Web. "+strDate+" &lt;<a href='"+location.href.replace(location.hash,"")+"'>"+location.href.replace(location.hash,"")+"#"+imageData.hash+"</a>&gt;";
				    if (location.href.includes("view.cgi")) {
					jq('#citation').attr("data-content", matches[1]);
				    }
				}
                                // /#AH
